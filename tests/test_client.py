...
class TestGetResponse:
    @pytest.mark.asyncio
    async def test_get_response_success(self, mock_response):
...
class TestListResultsPaginated:
    @pytest.mark.asyncio
    async def test_list_results_paginated_single_page(self, mock_response):
    ...
    async def test_list_results_paginated_with_additional_params(self):
        ...
        with patch('rick_and_morty_async.client.get_response', new_callable=AsyncMock) as mock_get:
            ...
            # Check that additional_params are included in params
            call_kwargs = mock_get.call_args[1]
            assert 'status' in call_kwargs.get('params', {})


class TestListResults:
    @pytest.mark.asyncio
    async def test_list_results(self, mock_response):
        ...
        with patch('rick_and_morty_async.client.get_response', new_callable=AsyncMock) as mock_get:
            ...
            # Check that results were put in queue
            assert q.qsize() == 2
...
class TestListCharacters:
    @pytest.mark.asyncio
    async def test_list_characters(self, root_response, mock_response):
        """Test list_characters function."""
        q = Queue()
        with patch('rick_and_morty_async.client.get_response', new_callable=AsyncMock) as mock_get:
            # First call returns root, second returns character count
            mock_get.side_effect = [
                root_response,
                {'info': {'pages': 1}, 'results': mock_response['results']}
            ]
...
class TestListCharactersPaginated:
    @pytest.mark.asyncio
    async def test_list_characters_paginated(self, root_response, mock_response):
        ...
        with patch('rick_and_morty_async.client.get_response', new_callable=AsyncMock) as mock_get, \
            ...
            mock_get.return_value = root_response
...
class TestListLocations:
    @pytest.mark.asyncio
    async def test_list_locations(self, root_response, mock_response):
...
class TestListLocationsPaginated:
    @pytest.mark.asyncio
    async def test_list_locations_paginated(self, root_response, mock_response):
...
class TestListEpisodes:
    @pytest.mark.asyncio
    async def test_list_episodes(self, root_response, mock_response):
...
class TestListEpisodesPaginated:
    @pytest.mark.asyncio
    async def test_list_episodes_paginated(self, root_response, mock_response):
...
class TestGetData:
    @pytest.mark.asyncio
    async def test_get_data(self, root_response, mock_response):
        ...
        with patch('rick_and_morty_async.client.get_response', new_callable=AsyncMock) as mock_get, \
            ...
            # Should have queued results from all three endpoints
            assert q.qsize() > 0
